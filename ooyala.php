...
class Ooyala {
	...
	const v4_url_base = '//player.ooyala.com/static/v4/stable/latest/';
	 ...
	 */
	function admin_menu() {
		add_options_page( 'Ooyala', 'Ooyala', self::capability, __FILE__, array( $this, 'settings_screen' ) );
	}
			...
			<tr class="ooyala-v4-setting">
				...
				<td scope="row">
					...
					<div class="ad_plugin-pulse-options" <?php if( $option['ad_plugin'] !== 'pulse' ) echo 'style="display: none"'; ?>>
						...
						<p class="description"><?php echo wp_kses_post( __( 'Review the <a href="https://www.dropbox.com/s/70u5h4oen6jlnbo/Ooyala-HTML5playerV4-Pulseintegration_20160406.pdf?dl=0">Pulse integration reference</a> for details on acceptable parameters.', 'ooyala' ) ); ?></p>
	...
	function validate_settings( $settings ) {
		$current = get_option( self::settings_key, $this->settings_default );
	...
	function admin_enqueue() {
		...
		if( $pagenow === 'options-general.php' && isset( $_GET['page'] ) && basename( $_GET['page'] ) === basename( __FILE__ ) ) {
	...
	function admin_notices() {
		...
		if( $this->configured() || !current_user_can( 'manage_options' ) ||
		  ( $pagenow === 'options-general.php' && $_GET['page'] === $page ) ) {
			return;
		}
		?>
		<div class="update-nag">
			<?php echo wp_kses_post( sprintf( __( 'Your Ooyala API credentials are not configured in <a href="%s">Ooyala Settings</a>.', 'ooyala' ), esc_url( $url ) ) ); ?>
		</div>
		<?php
	}
